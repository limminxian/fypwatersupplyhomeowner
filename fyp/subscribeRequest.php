...
if( !empty($_POST['userID']) && 
    ...
    if ($connection) {
		...
		//check if homeowner is already subbed to another company
		$subbedCompany = null;
		$SQL = "SELECT SUBSCRIBE FROM HOMEOWNER WHERE ID = '".$userID."'";
		$Result = mysqli_query($connection, $SQL);
		if(mysqli_num_rows($Result) != 0){
			$subbedCompany = mysqli_fetch_row($Result)[0];
		...
		if($subbedCompany != null){
			...
			if(mysqli_query($connection, $updateUserSQL)){
				if(mysqli_query($connection, $insertSubscribeSQL)){
					$result = array("status" => "success", "message" => "Insert data successful");
				} else $result = array("status" => "failed", "message" => "failed inserting subscription, subscription failed")
			} else $result = array("status" => "failed", "message" => "failed updating user, edit profile failed");
		} else $result = array("status" => "unsub", "message" => "Please unsubscribe first", "subID" => $subbedCompany);
